...
- **Dual-Scenario Applicant Ingestion:** \* _Batch Ingestion:_ Accepts JSON arrays of pre-formatted candidate profiles.
  - _PDF Parsing:_ Accepts raw PDF resumes, extracts the text using `pdfreader`, and uses AI to map the unstructured data flawlessly into the strict Umurava Talent Profile Schema.
...
4.  **Run the development server:**
    ...
    npm run dev
    ```
    _(Assuming `ts-node-dev` or `nodemon` is configured in your package.json scripts)._
