...
import { faBell as farBell, faBuilding as farBuilding, faStar as farStar, faFileLines as farFileLines, faPenToSquare as farPenToSquare, faIdBadge as farIdBadge, faAlignLeft as farAlignLeft } from "@fortawesome/free-regular-svg-icons";
...
const DEFAULT_DIMENSIONS: DimensionState[] = [
  { name: "Technical Skills Match", desc: "Relevance of tech stack and tools", tag: "Technical Skills", color: "bg-[#eff6ff] text-[#2563eb]", icon: faCode, bgColor: "#eff6ff", iconColor: "#2563eb", points: 30 },
  { name: "Experience Relevance", desc: "Years and domain expertise", tag: "Experience", color: "bg-[#f0fdf4] text-[#16a34a]", icon: faBriefcase, bgColor: "#f0fdf4", iconColor: "#16a34a", points: 20 },
  { name: "Education Alignment", desc: "Degree/certification match", tag: "Education", color: "bg-[#fef9c3] text-[#92400e]", icon: faGraduationCap, bgColor: "#fef9c3", iconColor: "#92400e", points: 20 },
  { name: "Profile Completeness", desc: "Portfolio, links, activity", tag: "Profile", color: "bg-[#f5f3ff] text-[#7c3aed]", icon: faCircleUser, bgColor: "#f5f3ff", iconColor: "#7c3aed", points: 10 },
  { name: "Red Flag Indicators", desc: "Gaps, job hopping, etc", tag: "Red Flags", color: "bg-[#fee2e2] text-[#b91c1c]", icon: faTriangleExclamation, bgColor: "#fee2e2", iconColor: "#b91c1c", points: 20 },
];
...
export default function NewJobPage() {
  ...
  const [tags, setTags] = useState<string[]>(["React", "Node.js", "TypeScript", "AWS"]);
  const [manualDesc, setManualDesc] = useState("");
  const [tagInput, setTagInput] = useState("");
  
  // Step 2 state
  const [dimensions, setDimensions] = useState<DimensionState[]>(DEFAULT_DIMENSIONS);
  const [dealbreakers, setDealbreakers] = useState<string[]>(["5+ years experience", "Full stack (React + Node.js)", "Must be eligible to work remotely"]);
  const [niceToHaves, setNiceToHaves] = useState<string[]>(["GraphQL experience", "Open source contributions"]);
  ...
  const handleCreateJob = async () => {
    ...
    try {
      const payload = {
        title: jobTitle,
        rawJD: mode === "paste" ? jdText : `Title: ${jobTitle}\nDepartment: ${department}\nLocation: ${location}\nLevel: ${experienceLevel}\nRequirements: ${tags.join(", ")}\n\n${manualDesc}`,
      };
  ...
  const generateRubric = async (jobId: string) => {
    ...
    try {
      const response = await apiClient.post<{ job: Job }>(`/jobs/${jobId}/parse-rubric`);
  ...
  // Confirm rubric and create session
    ...
    <div className="flex min-h-screen bg-[#f4f7fe]">
      ...
      <aside className={`bg-[#2563eb] flex flex-col sticky top-0 h-screen flex-shrink-0 transition-all duration-300 z-20 ${sidebarCollapsed ? "w-16" : "w-60"}`}>
        <button onClick={() => setSidebarCollapsed(!sidebarCollapsed)} className="absolute top-[22px] -right-3 w-6 h-6 bg-white rounded-full flex items-center justify-center shadow-md border border-[#e2e8f0] text-[#2563eb] cursor-pointer text-xs">
          <FontAwesomeIcon icon={sidebarCollapsed ? faChevronDown : faChevronDown} className="rotate-90" />
        </button>
        ...
        <div className="px-2.5 py-0.5">
          ...
          <div onClick={() => router.push("/dashboard")} className="flex items-center gap-2.5 px-3.5 py-2 my-0.5 rounded-xl text-white hover:bg-white/15 font-medium text-[13px] cursor-pointer">
            <FontAwesomeIcon icon={faHouse} className="w-4" />
            {!sidebarCollapsed && <span>Dashboard</span>}
          </div>
          
          <div className="flex items-center gap-2.5 px-3.5 py-2 my-0.5 rounded-xl bg-white text-[#2563eb] font-bold cursor-pointer">
            <FontAwesomeIcon icon={faBriefcase} className="w-4" />
            {!sidebarCollapsed && <span>Jobs</span>}
            {!sidebarCollapsed && <span className="ml-auto bg-primary/20 text-primary text-[10px] px-2 py-0.5 rounded-xl font-bold">3</span>}
          </div>
          
          <div className="flex items-center gap-2.5 px-3.5 py-2 my-0.5 rounded-xl text-white hover:bg-white/15 font-medium text-[13px] cursor-pointer">
          ...
          <div onClick={() => router.push("/intelligence")} className="flex items-center gap-2.5 px-3.5 py-2 my-0.5 rounded-xl text-white hover:bg-white/15 font-medium text-[13px] cursor-pointer">
        ...
        <div className="mt-auto mx-2.5 mb-2 bg-white/15 rounded-xl p-3 border border-white/18 flex items-center gap-2.5 cursor-pointer hover:bg-white/20">
          ...
          {!sidebarCollapsed && (
            <>
              ...
              <FontAwesomeIcon icon={faChevronDown} className="text-white/70 text-[11px] rotate-[270deg]" />
            </>
          )}
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-[62px] bg-white flex items-center justify-between px-6 border-b border-[#e2e8f0] sticky top-0 z-10 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="bg-primary border-0 rounded-full py-2 px-[18px] font-bold text-xs text-white flex items-center gap-1.5 shadow-[0_4px_14px_-4px_rgba(37,99,235,0.45)] hover:bg-primary-dark">
            ...
            <div className="flex items-center gap-2 ml-0.5 py-1 pl-1.5 pr-3 rounded-full bg-white border border-[#e2e8f0] cursor-pointer hover:border-primary">
        ...
        <div className="p-6 pb-8 overflow-y-auto flex-1">
          <div className="text-[13px] text-[#475569] font-medium mb-1">{greeting}, Elvis · New Session</div>
          
          <div className="mb-[18px]">
            <h2 className="font-bold text-[26px] text-[#0f172a]">Create Session</h2>
            <p className="text-[#475569] text-[13px]">Define the job brief and set scoring weights.</p>
          </div>
          ...
          <div className="flex items-center justify-center max-w-[400px] mx-auto mb-6">
            <div className="flex items-center w-full">
              <div className="flex flex-col items-center">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all ${currentStep === 1 ? "bg-primary text-white border-2 border-primary" : "bg-[#10b981] text-white border-2 border-[#10b981]"}`}>
                  {currentStep === 1 ? "1" : <FontAwesomeIcon icon={faCheck} />}
                </div>
                <span className={`text-xs font-bold mt-1.5 ${currentStep === 1 ? "text-[#0f172a]" : "text-[#0f172a]"}`}>Job Brief</span>
              </div>
              <div className={`flex-1 h-0.5 mx-1 ${currentStep === 2 ? "bg-[#10b981]" : "bg-[#e2e8f0]"}`} />
              <div className="flex flex-col items-center">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all ${currentStep === 2 ? "bg-primary text-white border-2 border-primary" : "bg-white border-2 border-[#e2e8f0] text-[#94a3b8]"}`}>
                  2
                </div>
                <span className={`text-xs font-medium mt-1.5 ${currentStep === 2 ? "text-[#0f172a] font-bold" : "text-[#94a3b8]"}`}>Scoring Rubric</span>
              </div>
            </div>
          </div>
            ...
            <div className="w-full max-w-[720px] mx-auto">
              ...
              <div className="flex justify-end mb-3">
                <div className="inline-flex bg-[#f1f5f9] rounded-full p-0.5">
                  <button onClick={() => setMode("paste")} className={`px-4 py-1.5 rounded-full text-xs font-semibold ${mode === "paste" ? "bg-primary text-white" : "bg-transparent text-[#475569]"}`}>
                    Paste JD
                  </button>
                  <button onClick={() => setMode("manual")} className={`px-4 py-1.5 rounded-full text-xs font-semibold ${mode === "manual" ? "bg-primary text-white" : "bg-transparent text-[#475569]"}`}>
                    Manual Form
                  </button>
                </div>
              </div>
              ...
              {mode === "paste" && (
                <div className="bg-white rounded-2xl shadow-sm border border-[#e2e8f0] p-5 relative">
                  <h3 className="text-[22px] font-extrabold text-[#0f172a] mb-1 flex items-center">
                    <FontAwesomeIcon icon={farFileLines} className="text-[#2563eb] text-lg mr-2.5" />
                    Paste Job Description
                  </h3>
                  <p className="text-[13px] text-[#475569] mb-4 flex items-center">
                    <FontAwesomeIcon icon={faWandMagicSparkles} className="text-[#2563eb] text-[10px] mr-1.5" />
                    Gemini will extract role details and build a scoring rubric automatically.
                  </p>
                  ...
                  <div className="flex justify-between items-center mt-3">
                    <span className="text-xs text-[#94a3b8]">{jdText.length} characters</span>
                    <button 
                    ...
                    >
                      {isLoading ? (
                        <><FontAwesomeIcon icon={faSpinner} spin className="mr-1" /> Analyzing...</>
                      ) : (
                        <>Extract with Gemini <span className="inline-block animate-pulse">✦</span></>
                      )}
                    </button>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-[#e2e8f0] p-5 pb-6">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3 className="text-[22px] font-extrabold text-[#0f172a] mb-0.5 flex items-center">
                        <FontAwesomeIcon icon={farPenToSquare} className="text-[#2563eb] text-lg mr-2.5" />
                        Manual Job Details
                      </h3>
                      <p className="text-[13px] text-[#475569] flex items-center">
                        <FontAwesomeIcon icon={faPencil} className="text-[#2563eb] text-[10px] mr-1.5" />
                  ...
                  {extractionBannerVisible && (
                    <div className="bg-[#f0fdf4] border border-[#bbf7d0] rounded-xl p-2.5 mb-4 text-[13px] text-[#166534] flex items-center gap-2">
                      <FontAwesomeIcon icon={faCircleCheck} className="text-[#10b981]" />
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-semibold text-[#475569] mb-1">
                        <FontAwesomeIcon icon={farIdBadge} className="text-primary text-[11px] mr-1.5" />
                        Job Title *
                      </label>
                      <input 
                        type="text" 
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-[#475569] mb-1">
                        <FontAwesomeIcon icon={farBuilding} className="text-primary text-[11px] mr-1.5" />
                        Department
                      </label>
                      <input 
                        type="text" 
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-[#475569] mb-1">
                        <FontAwesomeIcon icon={faLocationDot} className="text-primary text-[11px] mr-1.5" />
                        Location
                      </label>
                      <input 
                        type="text" 
                      ...
                      <div className="flex gap-2 mt-2">
                        {["Remote", "Hybrid", "On-site"].map((loc) => (
                          ...
                          >
                            {loc === "Remote" && <FontAwesomeIcon icon={faWifi} className="text-[10px] mr-1" />}
                            {loc === "Hybrid" && <FontAwesomeIcon icon={faCodeBranch} className="text-[10px] mr-1" />}
                            {loc === "On-site" && <FontAwesomeIcon icon={farBuilding} className="text-[10px] mr-1" />}
                            {loc}
                          </button>
                        ))}
                      </div>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-[#475569] mb-1">
                        <FontAwesomeIcon icon={faChartSimple} className="text-primary text-[11px] mr-1.5" />
                        Experience Level
                      </label>
                      <div className="flex gap-2 flex-wrap">
                        {["Junior", "Mid", "Senior", "Lead"].map((exp) => (
                          ...
                          >
                            {exp === "Junior" && <FontAwesomeIcon icon={faSeedling} className="text-[10px] mr-1" />}
                            {exp === "Mid" && <FontAwesomeIcon icon={faBolt} className="text-[10px] mr-1" />}
                            {exp === "Senior" && <FontAwesomeIcon icon={farStar} className="text-[10px] mr-1" />}
                            {exp === "Lead" && <FontAwesomeIcon icon={faCrown} className="text-[10px] mr-1" />}
                            {exp}
                          </button>
                        ))}
                      </div>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-[#475569] mb-1">
                        <FontAwesomeIcon icon={faTags} className="text-primary text-[11px] mr-1.5" />
                        Key Requirements * (min 2)
                      </label>
                      <div className="border border-[#e2e8f0] rounded-xl p-2 flex flex-wrap gap-2 items-center">
                        {tags.map((tag, idx) => (
                          <span key={idx} className="bg-[#eff6ff] text-[#2563eb] text-xs font-medium px-2.5 py-1 rounded-full flex items-center gap-1">
                            {tag}
                            <FontAwesomeIcon icon={faTimes} className="cursor-pointer hover:text-red-500" onClick={() => removeTag(idx)} />
                          </span>
                        ))}
                        <input 
                          type="text" 
                      ...
                      <p className="text-[10px] text-[#94a3b8] mt-1">Max 10 tags</p>
                    </div>
                    
                    <div>
                      <label className="block text-xs font-semibold text-[#475569] mb-1">
                        <FontAwesomeIcon icon={farAlignLeft} className="text-primary text-[11px] mr-1.5" />
                  ...
                  <div className="flex justify-end mt-6">
                    ...
                    >
                      {isLoading ? (
                        <><FontAwesomeIcon icon={faSpinner} spin className="mr-1" /> Creating...</>
                      ) : (
            ...
            <div className="w-full max-w-[800px] mx-auto space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      <span className="px-2.5 py-1 bg-[#f1f5f9] text-[#475569] text-xs font-medium rounded-full">
                        <FontAwesomeIcon icon={farBuilding} className="text-[10px] mr-1" />{department}
                      </span>
                      <span className="px-2.5 py-1 bg-[#f1f5f9] text-[#475569] text-xs font-medium rounded-full">
                        <FontAwesomeIcon icon={faLocationDot} className="text-[10px] mr-1" />{location}
                      </span>
                      <span className="px-2.5 py-1 bg-[#f1f5f9] text-[#475569] text-xs font-medium rounded-full">
                        <FontAwesomeIcon icon={faChartSimple} className="text-[10px] mr-1" />{experienceLevel}
                      </span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="bg-[#f5f3ff] text-[#7c3aed] text-[10px] font-bold px-2.5 py-1 rounded-full border border-[#e9d5ff]">
                      <FontAwesomeIcon icon={faWandMagicSparkles} className="text-[8px] mr-1" />AI
                    </span>
                    <button onClick={() => setCurrentStep(1)} className="text-xs font-medium text-primary hover:underline flex items-center gap-1">
              ...
              <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
                <h3 className="text-lg font-extrabold text-[#0f172a] flex items-center">
                  <FontAwesomeIcon icon={faSliders} className="text-primary mr-2" />
                  Scoring Weights
                </h3>
                <p className="text-[13px] text-[#475569] mb-3">Allocate 100 points across the five dimensions.</p>
                
                <div className="flex justify-center mb-4">
                  <div className={`inline-flex items-center px-4 py-1.5 rounded-full text-sm font-semibold ${
                    isBalanced ? "bg-[#ecfdf5] text-[#10b981]" : 
                    remainingPoints < 0 ? "bg-[#fee2e2] text-[#b91c1c]" : 
                    "bg-[#f1f5f9] text-[#475569]"
                  }`}>
                    {isBalanced ? (
                      <><FontAwesomeIcon icon={faCircleCheck} className="mr-1.5" /> Balanced — ready to confirm</>
                    ) : remainingPoints < 0 ? (
                ...
                <div className="space-y-3">
                  {dimensions.map((dim, idx) => {
                    ...
                    return (
                      <div key={idx} className="bg-white border border-[#e2e8f0] rounded-xl p-4 flex items-center justify-between gap-3">
                        <div className="flex items-center flex-1 min-w-0">
                          <div style={{ width: 36, height: 36, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginRight: 12, background: dim.bgColor, color: dim.iconColor }}>
                            <FontAwesomeIcon icon={dim.icon} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="font-bold text-[15px] text-[#0f172a]">{dim.name}</div>
                            <div className="flex items-center gap-1.5">
                              {editingDimensionIdx === idx ? (
                                ...
                                <>
                                  <span className="text-xs text-[#475569]">{dim.desc}</span>
                                  <FontAwesomeIcon 
                                    icon={faPencil} 
                            ...
                            <span className={`inline-block mt-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold ${dim.color}`}>
                              {dim.tag}
                            </span>
                          </div>
                        </div>
                        <div className="flex items-center gap-1 flex-shrink-0">
                          ...
                          <div className="flex gap-[3px] mx-1">
                            {Array.from({ length: 10 }).map((_, i) => (
                              <div key={i} className={`w-7 h-7 rounded-md ${i < segments ? "bg-primary" : "bg-[#f1f5f9]"}`} />
                            ))}
                          </div>
                          <button 
                            onClick={() => updateDimensionPoints(idx, 10)}
                          ...
                          <span className="font-bold text-[15px] text-[#0f172a] min-w-[50px] text-right">{dim.points} pts</span>
                        </div>
                      </div>
                    );
                  })}
                ...
                <div className="mt-5 bg-white rounded-2xl border border-[#e2e8f0] p-4">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-base font-extrabold text-[#0f172a]">Dealbreakers</h3>
                    <span className="bg-[#f5f3ff] text-[#7c3aed] text-[10px] font-bold px-2 py-0.5 rounded-full border border-[#e9d5ff]">
                      <FontAwesomeIcon icon={faWandMagicSparkles} className="text-[8px] mr-1" />AI
                    </span>
                  </div>
                  <p className="text-xs text-[#475569] mb-3">Requirements that automatically disqualify a candidate.</p>
                  
                  <div className="flex flex-wrap gap-2 mb-3">
                    {dealbreakers.map((item, i) => (
                      <span key={i} className="inline-flex items-center bg-[#fee2e2] text-[#b91c1c] border border-[#fecaca] text-xs font-semibold px-3 py-1.5 rounded-full">
                        {item}
                        <FontAwesomeIcon icon={faTimes} className="ml-2 cursor-pointer hover:text-red-700" onClick={() => removeDealbreaker(i)} />
                      </span>
                    ))}
                  </div>
                  ...
                  {showDealbreakerInput ? (
                    <div className="flex items-center gap-2">
                      <input 
                        ...
                        autoFocus
                      />
                      <button onClick={addDealbreaker} className="bg-primary text-white rounded-full w-7 h-7 flex items-center justify-center text-xs">
                        <FontAwesomeIcon icon={faCheck} />
                      </button>
                    </div>
                  ) : (
                    <button onClick={() => setShowDealbreakerInput(true)} className="text-xs font-semibold text-primary flex items-center gap-1 hover:underline">
                      <FontAwesomeIcon icon={faPlus} className="text-[10px]" /> Add dealbreaker
                    </button>
                  )}
                </div>
                ...
                <div className="mt-4 bg-white rounded-2xl border border-[#e2e8f0] p-4">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-base font-extrabold text-[#0f172a]">Nice to Have</h3>
                    <span className="bg-[#f5f3ff] text-[#7c3aed] text-[10px] font-bold px-2 py-0.5 rounded-full border border-[#e9d5ff]">
                      <FontAwesomeIcon icon={faWandMagicSparkles} className="text-[8px] mr-1" />AI
                    </span>
                  </div>
                  <p className="text-xs text-[#475569] mb-3">Skills that strengthen a candidate's profile.</p>
                  
                  <div className="flex flex-wrap gap-2 mb-3">
                    {niceToHaves.map((item, i) => (
                      <span key={i} className="inline-flex items-center bg-[#eff6ff] text-[#1d4ed8] border border-[#bfdbfe] text-xs font-semibold px-3 py-1.5 rounded-full">
                        {item}
                        <FontAwesomeIcon icon={faTimes} className="ml-2 cursor-pointer hover:text-blue-800" onClick={() => removeNiceToHave(i)} />
                      </span>
                    ))}
                  </div>
                  ...
                  {showNiceToHaveInput ? (
                    <div className="flex items-center gap-2">
                      <input 
                        ...
                        autoFocus
                      />
                      <button onClick={addNiceToHave} className="bg-primary text-white rounded-full w-7 h-7 flex items-center justify-center text-xs">
                        <FontAwesomeIcon icon={faCheck} />
                      </button>
                    </div>
                  ) : (
                    <button onClick={() => setShowNiceToHaveInput(true)} className="text-xs font-semibold text-primary flex items-center gap-1 hover:underline">
                      <FontAwesomeIcon icon={faPlus} className="text-[10px]" /> Add nice to have
                    </button>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center mt-5 pt-2">
                  <button onClick={() => setCurrentStep(1)} className="text-sm font-semibold text-[#475569] hover:text-primary">
                    <FontAwesomeIcon icon={faArrowLeft} className="mr-1.5" /> Back to Job Brief
                  </button>
                  <button 
                  ...
                  >
                    {isLoading ? (
                      <><FontAwesomeIcon icon={faSpinner} spin className="mr-1.5" /> Saving...</>
                    ) : isGeneratingRubric ? (
                      <><FontAwesomeIcon icon={faSpinner} spin className="mr-1.5" /> Generating AI Rubric...</>
                    ) : (
                      <><FontAwesomeIcon icon={faLock} className="mr-1.5" /> Confirm Rubric & Start Session</>
                    )}
                  </button>
                </div>
              </div>
