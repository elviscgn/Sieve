...
export default function JobApplicantsPage() {
  ...
  const [selectedPoolCandidates, setSelectedPoolCandidates] = useState<
    string[]
  >([]);
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const displayCandidates: CandidateDisplay[] = applicants.map(
          (app, idx) => {
            ...
            return {
              ...
              scoreColor,
              confidence:
                evalData?.confidence === "high"
                  ? "High Confidence"
                  : evalData?.confidence === "medium"
                    ? "Medium Confidence"
                    : evalData?.confidence === "low"
                      ? "Low Confidence"
                      : undefined,
              confidenceBg:
                evalData?.confidence === "high"
              ...
              dimensionScores: evalData
                ? {
                    tech:
                      evalData.dimensions.find((d) =>
                        d.name.includes("Technical"),
                      )?.score || 0,
                    exp:
                      evalData.dimensions.find((d) =>
                        d.name.includes("Experience"),
                      )?.score || 0,
                    edu:
                      evalData.dimensions.find((d) =>
                        d.name.includes("Education"),
                      )?.score || 0,
                    profile:
                      evalData.dimensions.find((d) =>
                        d.name.includes("Profile"),
                      )?.score || 0,
                    rf:
                      evalData.dimensions.find((d) =>
                        d.name.includes("Red Flag"),
                      )?.score || 0,
                  }
                : undefined,
        ...
        const evaluated = displayCandidates.filter(
  ...
  const handleRunScreening = async () => {
    ...
    try {
      ...
      const eventSource = new EventSource(
        `${process.env.NEXT_PUBLIC_API_URL}/sessions/${jobId}/stream`,
      );
    ...
    setChatMessages([
      {
        role: "user",
        content:
          candidate.qaQ || `What is ${candidate.name}'s biggest strength?`,
      },
      {
        role: "ai",
        content:
          candidate.qaA ||
          "Based on the evaluation, this candidate shows strong technical aptitude.",
      },
    ]);
    setPanelOpen(true);
  ...
  const handleSendChat = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/applicants/${selectedCandidate.id}/ask`,
        {
          method: "POST",
      ...
      setChatMessages((prev) => [
        ...prev,
        { role: "ai", content: "Sorry, I couldn't process that question." },
      ]);
    } finally {
    ...
    <div className="flex min-h-screen bg-[#f4f7fe]">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-6 pb-8 overflow-y-auto flex-1">
          ...
          {/* Status Banner */}
          {screeningProgress.evaluated > 0 &&
            screeningProgress.evaluated < screeningProgress.total && (
              <div className="bg-[#fffbeb] border border-[#fde68a] rounded-2xl p-5 mb-6 flex flex-wrap items-center justify-between">
                <div>
                  <div className="flex items-center gap-2 text-[#92400e] font-medium">
                    ...
                    />
                    <span>
                      Screening in progress — {screeningProgress.evaluated} of{" "}
                      {screeningProgress.total} candidates evaluated
                    </span>
                  </div>
                  <p className="text-xs text-[#b45309] mt-1">
                    Started recently · Est. completion soon
                  </p>
                </div>
                <div className="w-48 mt-2 sm:mt-0">
                  <div className="h-2 bg-[#fef3c7] rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(screeningProgress.evaluated / screeningProgress.total) * 100}%`,
                      }}
                    />
                  </div>
                </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="p-3 space-y-2 max-h-[600px] overflow-y-auto custom-scrollbar">
                  {filteredCandidates.length === 0 ? (
                    <div className="text-center py-12">
                      <FontAwesomeIcon
                        icon={faUserGroup}
                        className="text-4xl text-[#94a3b8] mb-3"
                      />
                    ...
                    filteredCandidates.map((candidate, idx) => (
                      ...
                      >
                        ...
                        <div
                          className="w-16 h-16 rounded-full border-3 flex flex-col items-center justify-center flex-shrink-0"
                          ...
                          {candidate.compositeScore ? (
                            <>
                              <span
                                ...
                                style={{ color: candidate.scoreColor }}
                              >
                                {candidate.compositeScore}
                              </span>
            ...
            <div className="w-full lg:w-80 flex-shrink-0 space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5">
                ...
                >
                  <FontAwesomeIcon
                    icon={rubricExpanded ? faChevronUp : faChevronDown}
                    className="text-xs"
                  />
                  View Full Rubric
                </button>
                    ...
                    {job.rubric.dimensions.map((dim, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center text-xs">
                          <span className="w-20 text-[#64748b] truncate">
                            {dim.name.split(" ")[0]}
                          </span>
      ...
      {showAddModal && (
        ...
        >
          <div
            className="bg-white rounded-2xl p-6 max-w-[550px] w-[90%] max-h-[80vh] overflow-auto"
            ...
            {addModalTab === "pool" ? (
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          ...
                          setSelectedPoolCandidates(
                            selectedPoolCandidates.filter((id) => id !== c.id),
                          );
                        }
                      }}
                    />
              ...
              >
                <FontAwesomeIcon
                  ...
                  className="text-3xl text-[#94a3b8] mb-2"
                />
                <p className="text-sm">
                  Drag & drop or{" "}
                  <button
      ...
      {panelOpen && selectedCandidate && (
        <div
          className="fixed inset-0 z-50 flex items-end justify-center bg-[#0f172a]/50 backdrop-blur-sm"
          onClick={closePanel}
        >
          <div
          ...
          >
            ...
            <div className="flex justify-end px-5 pb-2 flex-shrink-0">
              <button
                onClick={closePanel}
                className="w-8 h-8 rounded-full border border-[#e2e8f0] bg-white text-[#475569] flex items-center justify-center"
              >
                <FontAwesomeIcon icon={faTimes} />
              </button>
            </div>
            ...
            <div className="flex flex-1 min-h-0 overflow-hidden">
              ...
              <div className="w-[260px] flex-shrink-0 border-r border-[#f1f5f9] p-6 flex flex-col items-center bg-[#fafbff] overflow-y-auto custom-scrollbar">
                <div
                  ...
                  style={{
                    backgroundColor: selectedCandidate.avatarBg,
                    color: selectedCandidate.avatarColor,
                  }}
                >
                  {selectedCandidate.initials}
                </div>
                <h2 className="text-lg font-black tracking-tight text-[#0f172a] mt-3 text-center">
                  {selectedCandidate.name}
                </h2>
                <p className="text-xs text-[#94a3b8] mt-0.5 text-center">
                ...
                <div className="flex flex-wrap gap-1.5 mt-2.5 justify-center">
                  {selectedCandidate.skills.map((skill, i) => (
                    <span
                      key={i}
                      className="border border-[#e2e8f0] bg-white text-[#475569] text-[11px] font-semibold px-2.5 py-1 rounded-full"
                    >
                      {skill}
                    </span>
                  ))}
                ...
                <div className="text-center">
                  <span
                    ...
                    style={{ color: selectedCandidate.scoreColor }}
                  >
                    {selectedCandidate.compositeScore}
                  </span>
                  <span className="text-base text-[#94a3b8] font-normal align-super">
                    /100
                  </span>
                </div>
                ...
                <div
                  ...
                  style={{
                    backgroundColor: selectedCandidate.confidenceBg,
                    color: selectedCandidate.confidenceText,
                  }}
                >
              ...
              <div className="flex-1 flex flex-col min-w-0 overflow-y-auto custom-scrollbar">
                  ...
                  {selectedCandidate.dimensionScores &&
                    (() => {
                      const dims = [
                        {
                          name: "Technical",
                          score: selectedCandidate.dimensionScores.tech,
                          color: "#2563eb",
                          bg: "#eff6ff",
                          icon: faCode,
                        },
                        {
                          name: "Experience",
                          score: selectedCandidate.dimensionScores.exp,
                          color: "#16a34a",
                          bg: "#f0fdf4",
                          icon: faBriefcase,
                        },
                        {
                          name: "Education",
                          score: selectedCandidate.dimensionScores.edu,
                          color: "#d97706",
                          bg: "#fef9c3",
                        ...
                        {
                          name: "Profile Fit",
                          score: selectedCandidate.dimensionScores.profile,
                          color: "#7c3aed",
                          bg: "#f5f3ff",
                          icon: faUser,
                        },
                        {
                          name: "Red Flags",
                          score: selectedCandidate.dimensionScores.rf,
                          color: "#dc2626",
                          bg: "#fee2e2",
                          icon: faFlag,
                        },
                      ...
                      return dims.map((dim, i) => (
                        <div
                          key={i}
                          className="border border-[#f1f5f9] rounded-xl p-3 mb-2 bg-white"
                        >
                ...
                <div className="p-6 grid grid-cols-2 gap-3">
                  <div>
                    <div className="text-[11px] font-bold uppercase tracking-wider text-[#10b981] mb-2">
                      Strengths
                    </div>
                    {selectedCandidate.strengths?.map((s, i) => (
                      <div
                        key={i}
                        className="bg-[#f0fdf4] border border-[#bbf7d0] rounded-lg p-2.5 mb-1.5 text-xs text-[#166534] flex items-center gap-2"
                      >
                        <FontAwesomeIcon
                          icon={faCheck}
                          className="text-[10px]"
                  ...
                  <div>
                    <div className="text-[11px] font-bold uppercase tracking-wider text-[#f59e0b] mb-2">
                      Gaps
                    </div>
                    {selectedCandidate.gaps?.map((g, i) => (
                      <div
                        key={i}
                        className="bg-[#fffbeb] border border-[#fde68a] rounded-lg p-2.5 mb-1.5 text-xs text-[#92400e] flex items-center gap-2"
                      >
                        <FontAwesomeIcon
                          icon={faTimes}
                          className="text-[10px]"
                ...
                <div className="p-6">
                  <div className="bg-[#f8fafc] border border-[#e2e8f0] rounded-xl p-4">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="bg-[#eff6ff] text-[#2563eb] text-[10px] font-bold px-2 py-0.5 rounded-full">
                        ✦ AI
                      </span>
