...
export default function IntelligencePage() {
  ...
  const shouldShowEmptyState =
    showEmptyState || (data && data.totalSessions < 3);
            ...
            <div className="flex flex-col items-center justify-center py-16 px-4">
              <div className="bg-white rounded-3xl p-10 max-w-md w-full text-center shadow-sm border border-[#e2e8f0]">
                ...
                <h2 className="text-2xl font-bold text-[#0f172a] mb-2">
                  Intelligence unlocks after 3 sessions
                </h2>
                <p className="text-[#475569] text-sm mb-6">
                  Run your first few screenings and patterns will appear here
                  automatically.
                </p>
                <Link
                  href="/jobs/new"
                  className="bg-primary text-white px-6 py-2.5 rounded-full font-semibold text-sm shadow-sm hover:bg-primary-dark transition"
                >
                  Start first session →
                </Link>
                <div className="mt-5">
                  <button
                    ...
                    className="text-primary text-xs font-medium underline hover:no-underline"
                  >
            ...
            <>
              ...
              <div className="flex flex-wrap items-center justify-between mb-2">
                <div>
                  ...
                  <p className="text-[#475569] text-[13px] flex items-center gap-2 flex-wrap">
                    <span>
                      Patterns synthesised across {data.totalSessions} screening
                      sessions · Last refreshed{" "}
                      <span id="refreshTimestamp">{refreshTimestamp}</span>
                    </span>
                ...
                <button
                  ...
                  className="bg-transparent border border-[#e2e8f0] rounded-full py-1.5 px-5 font-semibold text-xs text-[#0f172a] flex items-center gap-2 cursor-pointer hover:border-primary hover:text-primary transition-all"
                >
                  <FontAwesomeIcon
                    icon={faRotate}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-white rounded-2xl p-5 border border-[#e2e8f0] shadow-sm animate-fadeUp [animation-delay:150ms]">
                  ...
                  <div className="text-[20px] font-extrabold text-[#0f172a] leading-tight">
                    {data.mostScreenedRole}
                  </div>
                  <div className="text-xs text-[#475569]">
                    Most Screened Role
                  </div>
              ...
              <div className="bg-white rounded-2xl p-5 border border-[#e2e8f0] shadow-sm mb-6">
                ...
                <p className="text-xs text-[#475569] mb-3">
                  Match rate = candidates scoring above 65 composite ÷ total
                ...
                <div className="mt-4 bg-[#eff6ff] border border-[#bfdbfe] rounded-xl p-3 flex items-start gap-3">
                  <FontAwesomeIcon
                    icon={faArrowTrendUp}
                    className="text-[#2563eb] text-lg mt-0.5"
                  />
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
                ...
                {data.insights.skillGap && (
                  <div className="bg-white rounded-2xl p-5 border border-[#e2e8f0] shadow-sm flex flex-col">
                    ...
                    <span className="inline-block text-[11px] font-semibold px-3 py-1 rounded-full bg-[#eff6ff] text-[#1d4ed8] w-fit mb-2">
                      Skill Gap
                    </span>
                    <h4 className="font-bold text-base mb-1">
                      {data.insights.skillGap.title}
                    </h4>
                    <p className="text-[13px] text-[#475569] leading-relaxed mb-3">
                      {data.insights.skillGap.description}
                    </p>
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {data.insights.skillGap.affectedRoles.map((role, i) => (
                        <span
                          key={i}
                          className="bg-[#f1f5f9] text-[#475569] text-[11px] px-2.5 py-1 rounded-full"
                        >
                          {role}
                        </span>
                      ))}
                ...
                {data.insights.matchRate && (
                  <div className="bg-white rounded-2xl p-5 border border-[#e2e8f0] shadow-sm flex flex-col">
                    ...
                    <span className="inline-block text-[11px] font-semibold px-3 py-1 rounded-full bg-[#fef9c3] text-[#854d0e] w-fit mb-2">
                      Match Rate
                    </span>
                    <h4 className="font-bold text-base mb-1">
                      {data.insights.matchRate.title}
                    </h4>
                    <p className="text-[13px] text-[#475569] leading-relaxed mb-3">
                      {data.insights.matchRate.description}
                    </p>
                    <div className="flex flex-wrap gap-1.5 mb-4">
                ...
                {data.insights.requirementMismatch && (
                  <div className="bg-white rounded-2xl p-5 border border-[#e2e8f0] shadow-sm flex flex-col">
                    ...
                    <span className="inline-block text-[11px] font-semibold px-3 py-1 rounded-full bg-[#f5f3ff] text-[#6d28d9] w-fit mb-2">
                      Requirement Mismatch
                    </span>
                    <h4 className="font-bold text-base mb-1">
                      {data.insights.requirementMismatch.title}
                    </h4>
                    <p className="text-[13px] text-[#475569] leading-relaxed mb-3">
                      {data.insights.requirementMismatch.description}
                    </p>
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {data.insights.requirementMismatch.affectedRoles.map(
                        (role, i) => (
                          <span
                            key={i}
                            className="bg-[#f1f5f9] text-[#475569] text-[11px] px-2.5 py-1 rounded-full"
                    ...
                    <div className="bg-[#f8fafc] rounded-xl p-3 border-l-[3px] border-l-[#7c3aed] text-[13px] text-[#475569] italic flex gap-2">
                      <FontAwesomeIcon
                        icon={faLightbulb}
                        className="text-[#7c3aed] mt-0.5"
                      />
                      <span>
                        {data.insights.requirementMismatch.recommendation}
                      </span>
                    </div>
                  </div>
                )}
              ...
              <div className="bg-white rounded-2xl p-5 border border-[#e2e8f0] shadow-sm mb-6">
                ...
                <p className="text-xs text-[#475569] mb-3">
                  Skills most frequently absent across all screened candidates
                </p>
                    ...
                    options={{
                      ...
                      plugins: {
                        ...
                        tooltip: {
                          callbacks: {
                            label: (ctx) =>
                              `${ctx.raw} candidates missing this skill`,
                          },
                        },
                      },
                      scales: {
                        x: {
                          min: 0,
                          max:
                            Math.max(...data.topSkillGaps.map((g) => g.count)) +
                            2,
                          grid: { color: "#f1f5f9" },
                        },
                ...
                <p className="text-xs text-[#94a3b8] mt-3">
                  Based on {data.totalCandidates} candidates across{" "}
                  {data.totalSessions} sessions. Missing = scored below 50 on
                  the relevant dimension keyword.
                </p>
              </div>
              ...
              <div className="bg-white rounded-2xl p-5 border border-[#e2e8f0] shadow-sm mb-6 overflow-x-auto">
                ...
                <p className="text-xs text-[#475569] mb-4">
                  All screening sessions run through Sieve
                </p>
                <table className="w-full text-[13px] border-collapse min-w-[900px]">
                  ...
                  <tbody>
                    {data.sessionHistory.map((session, idx) => (
                      <tr
                        key={session.id}
                        className={`border-b border-[#f1f5f9] ${idx % 2 === 0 ? "bg-white" : "bg-[#f8fafc]"} hover:bg-[#f1f5f9]`}
                      >
                        ...
                        <td
                          className={`py-3 px-2 font-semibold ${session.avgScore >= 70 ? "text-[#10b981]" : "text-[#f59e0b]"}`}
                        >
                          {session.avgScore}
                        </td>
                        ...
                        <td className="py-3 px-2">
                          <span
                            className={`px-2 py-1 rounded-full text-[10px] font-bold ${
                              session.topFlag === "Overqualified"
                                ? "bg-[#fff7ed] text-[#c2410c]"
                                : session.topFlag === "SkillMismatch"
                        ...
                        <td className="py-3 px-2">
                          <Link
                            ...
                            className="bg-[#eff6ff] text-[#2563eb] rounded-full px-3 py-1 text-xs font-semibold hover:bg-[#2563eb] hover:text-white transition"
                          >
                            View →
                          </Link>
                        </td>
              ...
              <div className="rounded-2xl p-6 md:p-8 border border-[#bfdbfe] shadow-sm flex flex-col md:flex-row gap-6 items-center bg-gradient-to-br from-[#eff6ff] to-[#f5f3ff]">
                <div className="flex-1">
                  <span className="bg-[#2563eb] text-white text-[11px] font-bold px-4 py-1 rounded-full inline-block mb-3">
                    Built for Umurava
                  </span>
                  <h3 className="font-bold text-xl md:text-2xl mb-2">
                    Intelligence grows with every session
                  </h3>
                  <p className="text-[#475569] text-sm leading-relaxed mb-4">
                    Every screening session Sieve runs adds to a growing dataset
                    of talent patterns specific to your Umurava pool. Over time,
                    Sieve learns which roles attract the strongest matches,
                    which skills are chronically missing, and which rubric
                    configurations produce the best hires.
                  </p>
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      />{" "}
                      Patterns are private to your organisation
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <FontAwesomeIcon
                        icon={faCheckCircle}
                        className="text-[#10b981]"
                      />{" "}
                      Intelligence improves automatically — no manual input
                      needed
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-4 text-right">
                <button
                  ...
                  className="text-xs text-[#94a3b8] underline hover:text-[#2563eb]"
                >
