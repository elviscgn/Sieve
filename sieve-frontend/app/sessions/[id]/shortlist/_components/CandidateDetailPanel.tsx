...
export default function CandidateDetailPanel({
  ...
  const handleSendMessage = async (question: string) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/applicants/${candidate.id}/ask`, {
        method: "POST",
            ...
            try {
              ...
              if (data.text) {
                ...
                setChatMessages(prev => {
                  ...
                  newMessages[newMessages.length - 1] = { role: "ai", content: accumulated };
                  return newMessages;
                });
              }
              if (data.done) break;
      ...
      const streamInterval = setInterval(() => {
        if (streamed.length < fallback.length) {
          ...
          setChatMessages(prev => {
            const newMessages = [...prev];
            newMessages[newMessages.length - 1] = { role: "ai", content: streamed };
            return newMessages;
          });
        } else {
  ...
  const dimensionLabels = ["Technical Skills Match", "Experience Relevance", "Education Alignment", "Profile Completeness", "Red Flag Indicators"];
  const dimensionKeys = ["tech", "exp", "edu", "prof", "flags"] as const;
  const dimensionIcons = [faCode, faBriefcase, faGraduationCap, faUserCheck, faFlag];
  const dimensionColors = ["#2563eb", "#f59e0b", "#8b5cf6", "#f97316", "#ef4444"];
      ...
      <div className="fixed inset-y-0 right-0 w-full max-w-[900px] bg-[#f4f7fe] shadow-2xl z-50 transform transition-transform duration-300 ease-out flex flex-col">
        ...
        <div className="h-[62px] bg-white flex items-center justify-between px-6 border-b border-[#e2e8f0] flex-shrink-0">
          <div className="flex items-center gap-3">
            <button onClick={onClose} className="w-8 h-8 rounded-full border border-[#e2e8f0] flex items-center justify-center hover:border-primary hover:text-primary transition">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-full border border-[#e2e8f0] flex items-center justify-center hover:border-primary">
            ...
            <div className="w-[320px] flex-shrink-0">
              <div className="bg-white rounded-2xl shadow-sm border border-[#f1f5f9] p-5 sticky top-6 flex flex-col gap-5">
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  <h2 className="font-bold text-[22px] text-[#0f172a]">{candidate.name}</h2>
                  <p className="text-sm text-[#475569]">{candidate.role} · {candidate.exp}</p>
                  
                  <div className="flex flex-wrap justify-center gap-1.5 mt-2">
                    {candidate.skills.slice(0, 5).map(skill => (
                      <span key={skill} className="bg-[#eff6ff] text-[#2563eb] text-[11px] font-medium px-2.5 py-1 rounded-full">
                        {skill}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-3 mt-3 text-xs">
                    ...
                    <span className="text-[#475569] flex items-center gap-1">
                      <FontAwesomeIcon icon={faLocationDot} /> {candidate.location || "Johannesburg, ZA"}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center border-t border-b border-[#f1f5f9] py-5">
                  <div 
                    ...
                    style={{ borderColor: candidate.composite >= 80 ? "#10b981" : candidate.composite >= 60 ? "#f59e0b" : "#ef4444" }}
                  >
                    <span 
                      className="text-4xl font-extrabold"
                      style={{ color: candidate.composite >= 80 ? "#10b981" : candidate.composite >= 60 ? "#f59e0b" : "#ef4444" }}
                    >
                      {candidate.composite}
                    </span>
                  </div>
                  <span className="text-xs font-medium text-[#475569] uppercase tracking-wider">Composite Score</span>
                  <div className="flex gap-2 mt-3">
                    <span 
                      className="px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1"
                      style={{ 
                        background: candidate.confidence === "High" ? "#dcfce7" : "#fef9c3", 
                        color: candidate.confidence === "High" ? "#166534" : "#92400e",
                        border: `1px solid ${candidate.confidence === "High" ? "#bbf7d0" : "#fde68a"}`
                      }}
                    >
                      <FontAwesomeIcon icon={faChartSimple} /> {candidate.confidence} Confidence
                    </span>
                    {candidate.flags.length > 0 && (
                      ...
                      >
                        <FontAwesomeIcon icon={faTriangleExclamation} /> {candidate.flags[0]}
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <button onClick={onClose} className="w-full py-2.5 border border-[#e2e8f0] rounded-xl font-semibold text-sm text-[#0f172a] hover:border-primary hover:text-primary transition">
                    <FontAwesomeIcon icon={faArrowLeft} className="mr-1" /> Back to Shortlist
                  </button>
                  ...
                  <button className="w-full py-2 text-[#475569] text-sm hover:text-primary transition">
                    <FontAwesomeIcon icon={faDownload} className="mr-1" /> Export profile
                  </button>
                </div>
                
                {/* Session context */}
                <div className="bg-[#f8fafc] rounded-xl p-4 text-xs border border-[#e2e8f0]">
                  <p className="font-bold text-[#0f172a]">This session: Senior Full Stack Engineer</p>
                  <p className="text-[#475569] mt-1">Rubric confirmed Apr 14 2026</p>
                  <p className="text-[#475569]">Screened against 5 other candidates</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col gap-5 min-w-0">
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#f1f5f9]">
                ...
                <div className="space-y-5">
                  {dimensionKeys.map((key, idx) => {
                    ...
                    return (
                      <div key={key} className={isRedFlag ? "p-3 rounded-xl bg-[#fff1f2]" : ""}>
                        <div className="flex items-center gap-2 mb-1">
                          <FontAwesomeIcon icon={dimensionIcons[idx]} style={{ color: dimensionColors[idx] }} />
                          <span className="font-bold text-sm">{dimensionLabels[idx]}</span>
                          <span className="ml-auto font-bold text-lg" style={{ color: scoreColors[idx] }}>{score}</span>
                        </div>
                        <div className="w-full h-1.5 bg-[#f1f5f9] rounded-full">
                          <div className="h-full rounded-full" style={{ width: `${score}%`, backgroundColor: dimensionColors[idx] }} />
                        </div>
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#f1f5f9]">
                <div className="grid grid-cols-2 gap-6">
                    ...
                    <ul className="space-y-2">
                      {candidate.strengths.map((s, i) => (
                        <li key={i} className="flex gap-2 text-[13px]">
                          <FontAwesomeIcon icon={faCheck} className="text-[#10b981] mt-1" />
                          <span>{s}</span>
                        </li>
                      ))}
                    </ul>
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-3">
                      <FontAwesomeIcon icon={faTriangleExclamation} className="text-[#ef4444]" />
                    ...
                    <ul className="space-y-2">
                      {candidate.gaps.map((g, i) => (
                        <li key={i} className="flex gap-2 text-[13px]">
                          <FontAwesomeIcon icon={faXmark} className="text-[#ef4444] mt-1" />
                          <span>{g}</span>
                        </li>
                      ))}
                    </ul>
              ...
              <div className="rounded-2xl p-5 border bg-[#f5f3ff] border-[#e9d5ff]">
                <div className="flex items-center gap-2 mb-3">
                  <h4 className="font-bold text-base text-[#3b0764]">Recommendation</h4>
                  <span className="bg-[#f5f3ff] text-[#7c3aed] border border-[#e9d5ff] text-[10px] font-bold px-2 py-0.5 rounded-full inline-flex items-center gap-1">
                    <FontAwesomeIcon icon={faWandMagicSparkles} className="text-[8px]" /> AI
                  </span>
                </div>
                <p className="text-[14px] text-[#3b0764] mb-5">{candidate.aiRec}</p>
                <div className="flex gap-3">
                  <button className="px-5 py-2.5 bg-[#10b981] text-white rounded-xl font-semibold text-sm hover:bg-[#059669] transition">
                    <FontAwesomeIcon icon={faCheck} className="mr-1" /> Advance to interview
                  </button>
                  <button className="px-5 py-2.5 border border-[#ef4444] text-[#ef4444] rounded-xl font-semibold text-sm hover:bg-[#fef2f2] transition">
                    <FontAwesomeIcon icon={faXmark} className="mr-1" /> Pass on candidate
                  </button>
                </div>
              </div>
                    ...
                    {candidate.flags.map(flag => (
                      <div key={flag}>
                        ...
                        <p className="text-sm text-[#0f172a]">
                          {flag === "Overqualified" && "This candidate's experience significantly exceeds the target profile. Research suggests overqualified candidates have higher attrition risk."}
                          {flag === "Underexperienced" && "This candidate has less experience than the target range. They may require additional mentorship."}
                          {flag === "SkillMismatch" && "The candidate's primary skill set doesn't fully align with the job requirements."}
                          {flag === "Low Confidence" && "The AI has lower confidence in this evaluation due to incomplete or ambiguous profile data."}
                        </p>
                        <div className="mt-3 p-3 bg-[#f8fafc] rounded-lg italic text-sm text-[#475569]">
                          <FontAwesomeIcon icon={faCircleCheck} className="mr-1 text-primary" /> 
                          <strong>What to ask in interview:</strong> "Tell me about your long-term career goals."
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#f1f5f9]">
                <div className="flex items-center gap-2 mb-3">
                  <h4 className="font-bold text-base">Ask about this candidate</h4>
                  <span className="bg-[#f5f3ff] text-[#7c3aed] border border-[#e9d5ff] text-[10px] font-bold px-2 py-0.5 rounded-full inline-flex items-center gap-1">
                    <FontAwesomeIcon icon={faWandMagicSparkles} className="text-[8px]" /> AI
                  </span>
                </div>
                ...
                >
                  {chatMessages.map((msg, i) => (
                    ...
                    >
                      ...
                      {msg.content || (msg.role === "ai" && isStreaming && <FontAwesomeIcon icon={faSpinner} spin />)}
                    </div>
                  ))}
                </div>
                ...
                {showSuggestions && (
                  <div className="flex flex-wrap gap-2 mb-3">
                    <button 
                      onClick={() => handleSuggestionClick("What is the biggest hire risk?")}
                      className="px-3 py-1.5 bg-[#f1f5f9] hover:bg-[#e2e8f0] rounded-full text-xs font-medium transition"
                    >
                      What is the biggest hire risk?
                    </button>
                    <button 
                      onClick={() => handleSuggestionClick("Why ranked 3rd and not higher?")}
                      className="px-3 py-1.5 bg-[#f1f5f9] hover:bg-[#e2e8f0] rounded-full text-xs font-medium transition"
                    >
                      Why ranked 3rd and not higher?
                    </button>
                    <button 
                      onClick={() => handleSuggestionClick("One sentence summary for the hiring manager")}
                      className="px-3 py-1.5 bg-[#f1f5f9] hover:bg-[#e2e8f0] rounded-full text-xs font-medium transition"
                    >
