...
export default function ScreeningPage() {
  ...
  const [logs, setLogs] = useState<Array<{ message: string; type: string; icon: string }>>([]);
  ...
  // Add log entry
  const addLog = useCallback((message: string, type: "info" | "complete" | "evaluating" = "info", icon: string = "circle-info") => {
  ...
  useEffect(() => {
    const loadSession = async () => {
      try {
        ...
        const session = await apiClient.get<ScreeningSession>(`/sessions/${sessionId}`);
        const jobData = await apiClient.get<Job>(`/jobs/${session.jobId}`);
        setJob(jobData);
        setRubric(session.rubricSnapshot);

        // Fetch applicants for this job
        const applicants = await apiClient.get<Applicant[]>(`/jobs/${session.jobId}/applicants`);
          ...
          const updatedCandidates = initialCandidates.map((c) => {
            ...
            if (result) {
              return {
                ...
                color: result.compositeScore >= 70 ? "#10b981" : result.compositeScore >= 50 ? "#f59e0b" : "#ef4444",
                flags: result.flags,
              };
            }
            return c;
        ...
        addLog(`Screening session started — ${initialCandidates.length} candidates queued`, "info", "play");
  ...
  useEffect(() => {
    ...
    const eventSource = new EventSource(`${process.env.NEXT_PUBLIC_API_URL}/sessions/${sessionId}/stream`);
    ...
    eventSource.onmessage = (event) => {
      ...
      setCandidates((prev) =>
        prev.map((c) => {
          if (c.id === data.applicantId) {
            ...
            addLog(`${c.name} evaluated — score: ${score}`, "complete", "check-circle");
            return {
              ...c,
              status: "complete" as const,
              score,
      ...
      if (data.evaluatedCount === data.totalApplicants) {
        setIsComplete(true);
        addLog("Screening complete — all candidates evaluated", "complete", "flag-checkered");
      }
    };
  ...
  const animateScore = useCallback((element: HTMLSpanElement, target: number) => {
    const duration = 800;
    const start = 0;
  ...
  const registerScoreElement = useCallback((id: string, el: HTMLSpanElement | null) => {
    if (el) {
  ...
  const renderCandidateCard = (candidate: CandidateDisplay) => {
    if (candidate.status === "pending") {
      return (
        <div key={candidate.id} className="candidate-card bg-[#fafafa] rounded-2xl border border-[#f1f5f9] p-5 opacity-80">
          <div className="flex items-start gap-5">
            <div className="flex items-center gap-3 w-48 flex-shrink-0">
              <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold" style={{ backgroundColor: candidate.avatarBg, color: candidate.avatarColor }}>
                {candidate.initials}
              </div>
              <div>
                <div className="font-bold">{candidate.name}</div>
                <div className="text-xs text-[#64748b]">{candidate.role}</div>
                <span className="inline-block mt-1 text-[10px] font-semibold bg-[#f1f5f9] text-[#64748b] px-2 py-0.5 rounded-full">Queued</span>
              </div>
            </div>
            <div className="flex-1 flex flex-col gap-2">
              {Array(5).fill(0).map((_, i) => (
                <div key={i} className="bg-[#e2e8f0] h-1.5 rounded-full w-full" />
              ))}
            </div>
    ...
    if (candidate.status === "evaluating") {
      return (
        <div key={candidate.id} className="candidate-card evaluating-card bg-[#fafbff] rounded-2xl border-2 border-[#bfdbfe] p-5 relative overflow-hidden">
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="flex-1 flex flex-col gap-2 justify-center">
              {Array(5).fill(0).map((_, i) => (
                <div key={i} className="shimmer-bar h-1.5 rounded-full w-full" />
              ))}
            </div>
            <div className="w-20 flex-shrink-0 flex flex-col items-center">
              ...
              <span className="text-[10px] text-[#2563eb] font-medium mt-1">Processing</span>
            </div>
          </div>
        </div>
      );
    ...
    return (
      <div key={candidate.id} className="candidate-card bg-white rounded-2xl p-5 card-reveal relative border-l-4" style={{ borderLeftColor: color }}>
        <FontAwesomeIcon icon={faCircleCheck} className="absolute top-3 right-3 text-[#10b981] text-lg" />
        <div className="flex items-start gap-5">
          <div className="flex items-center gap-3 w-48 flex-shrink-0">
            <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold" style={{ backgroundColor: candidate.avatarBg, color: candidate.avatarColor }}>
              {candidate.initials}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {candidate.skills.map((skill, i) => (
                  <span key={i} className="text-[9px] bg-[#f1f5f9] px-2 py-0.5 rounded-full">{skill}</span>
                ))}
              </div>
            </div>
          </div>
          <div className="flex gap-3 flex-1 justify-center">
            {dimensionScores.map((s, i) => (
              <div key={i} className="flex flex-col items-center gap-1">
                <canvas id={`ring-${candidate.id}-${i}`} width="36" height="36" className="w-9 h-9" />
                <span className="text-[9px] font-bold uppercase tracking-wider" style={{ color: dimensionColors[i] }}>{dimensionLabels[i]}</span>
              </div>
            ))}
          </div>
          <div className="flex flex-col items-center justify-center w-20 flex-shrink-0">
            <div className="w-14 h-14 rounded-full border-4 flex flex-col items-center justify-center" style={{ borderColor: color }}>
              <span
        ...
        <div className="flex flex-wrap items-center gap-1.5 mt-3 pl-[124px]">
          {dimensionScores.map((s, i) => (
            <span key={i} className="dimension-pill text-[10px] font-bold px-2 py-1 rounded-full text-white" style={{ backgroundColor: dimensionColors[i], animationDelay: `${i * 0.15}s` }}>
              {dimensionLabels[i]} {s}
            </span>
          ))}
        </div>
        {flags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2 pl-[124px]">
            {flags.map((flag, i) => (
              <span key={i} className={`flag-tag text-[10px] px-2 py-0.5 rounded-full font-semibold ${
                flag === "Overqualified" ? "bg-[#fff7ed] text-[#c2410c]" :
                flag === "Underexperienced" ? "bg-[#f1f5f9] text-[#475569]" :
  ...
  const renderStatusDots = () => {
    return candidates.map((c) => {
      let dotClass = "w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs transition-all ";
      let content;
      if (c.status === "complete") {
        dotClass += "bg-[#10b981]";
        content = <FontAwesomeIcon icon={faCheck} className="text-white text-sm" />;
      } else if (c.status === "evaluating") {
        dotClass += "bg-[#2563eb]";
        content = <FontAwesomeIcon icon={faSyncAlt} spin className="text-white text-xs" />;
      } else {
  ...
  return (
    <div className="flex min-h-screen bg-[#f4f7fe]">
      {/* Sidebar - same as previous stages */}
        ...
        <div className="p-6 pb-8 overflow-y-auto flex-1">
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              <div className="text-sm mb-1">
                ...
                <Link href={`/sessions/${sessionId}`} className="text-[#2563eb] hover:underline font-medium">{job.title}</Link>
                <span className="text-[#94a3b8] mx-1.5">/</span>
                <span className="text-[#475569]">Screening</span>
              </div>
              <h1 className="font-bold text-[28px] text-[#0f172a] tracking-tight">Live Screening</h1>
              <p className="text-sm text-[#64748b] mt-1">AI is evaluating candidates against your scoring rubric in real time</p>
            </div>
            <div className={`flex items-center gap-2 rounded-full py-1.5 px-4 text-xs font-bold ${
              isComplete ? "bg-[#f0fdf4] text-[#166534] border border-[#bbf7d0]" : "bg-[#fef3c7] text-[#92400e] border border-[#fde68a]"
            }`}>
              {isComplete ? (
                <>
          ...
          <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5 mb-6 shadow-sm">
            <div className="flex justify-between items-start mb-3">
              <div>
                <div className="font-bold text-[15px]">{evaluatedCount} of {totalCandidates} evaluated</div>
                <div className="text-xs text-[#64748b] mt-0.5">
                  {totalCandidates - evaluatedCount} remaining · {isComplete ? "Complete" : `Est. ~${(totalCandidates - evaluatedCount) * 1.5} min`}
                </div>
              </div>
              <div className="text-[22px] font-black text-[#2563eb]">{progressPercent}%</div>
            </div>
            ...
            <div className="completion-banner-bg bg-gradient-to-r from-[#f0fdf4] to-[#eff6ff] border border-[#bbf7d0] rounded-2xl p-5 mb-5 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-base font-extrabold text-[#0f172a]">
                    <FontAwesomeIcon icon={faCircleCheck} className="text-[#10b981] mr-2" />
                    Screening complete — {totalCandidates} candidates evaluated
                  </div>
                  <div className="text-[13px] text-[#475569] mt-1">
                    Top candidate: {candidates.find(c => c.status === "complete")?.name || "—"} · Avg score: {Math.round(candidates.filter(c => c.status === "complete").reduce((acc, c) => acc + (c.score || 0), 0) / evaluatedCount) || 0}
                  </div>
                </div>
                <button
                  onClick={() => router.push(`/sessions/${sessionId}/shortlist`)}
              ...
              {isAutoRedirecting && (
                <div className="mt-3">
                  ...
                  <p className="text-xs text-[#475569] mt-2">Navigating to shortlist...</p>
                </div>
              )}
            </div>
          )}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-extrabold text-lg">
                  Candidates <span className="bg-[#e2e8f0] text-[#475569] text-xs px-2 py-0.5 rounded-full ml-2">{totalCandidates}</span>
                </h3>
              </div>
            ...
            <div className="w-full lg:w-72 flex-shrink-0 space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5 shadow-sm">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between"><span className="text-[#64748b]">Job</span><span className="font-medium">{job.title}</span></div>
                  <div className="flex justify-between"><span className="text-[#64748b]">Dept</span><span className="font-medium">Engineering</span></div>
                  <div className="flex justify-between"><span className="text-[#64748b]">Location</span><span className="font-medium">Remote</span></div>
                  <div className="flex justify-between"><span className="text-[#64748b]">Rubric</span><span className="text-[#10b981] font-medium"><FontAwesomeIcon icon={faCheck} className="text-xs mr-1" />Confirmed</span></div>
                  <div className="flex justify-between"><span className="text-[#64748b]">Candidates</span><span className="font-medium">{totalCandidates}</span></div>
                </div>
              </div>
              ...
              {rubric && (
                <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5 shadow-sm">
                  <div className="flex items-center gap-2 mb-1">
                    <FontAwesomeIcon icon={faWeightScale} className="text-[#64748b]" />
                    <h4 className="font-bold text-[#0f172a]">Confirmed Rubric</h4>
                  </div>
                  <div className="flex items-center gap-1 mt-1 mb-2">
                    <span className="bg-amber-50 text-amber-700 text-[10px] font-bold px-2 py-0.5 rounded-full">✦ AI</span>
                    <span className="text-[10px] text-[#64748b]">Recruiter-confirmed</span>
                  </div>
                  <div className="flex h-2 rounded-full overflow-hidden my-3">
                    {rubric.dimensions.map((dim, i) => (
                      <div key={i} style={{ width: `${dim.weight}%`, backgroundColor: dimensionColors[i % dimensionColors.length] }} />
                    ))}
                  </div>
                  <div className="flex flex-wrap gap-x-3 gap-y-1 text-[10px] font-medium">
                    {rubric.dimensions.map((dim, i) => (
                      <span key={i}>
                        <span style={{ color: dimensionColors[i % dimensionColors.length] }}>●</span> {dim.name.split(" ")[0]} {dim.weight}%
                      </span>
                    ))}
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl border border-[#e2e8f0] p-5 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                  <FontAwesomeIcon icon={faTerminal} className="text-[#64748b]" />
                ...
                <div className="space-y-2.5 max-h-[320px] overflow-y-auto custom-scrollbar pr-1">
                  {logs.map((log, i) => (
                    <div key={i} className="flex items-start gap-2.5 text-xs log-entry-new">
                      <span className="w-12 flex-shrink-0 text-[11px] text-[#94a3b8] font-medium">Just now</span>
                      <FontAwesomeIcon
                        icon={log.type === "complete" ? faCircleCheck : log.type === "evaluating" ? faSyncAlt : faCircleInfo}
                        className="text-xs mt-0.5"
                        style={{ color: log.type === "complete" ? "#10b981" : log.type === "evaluating" ? "#2563eb" : "#94a3b8" }}
                      />
